...
exports.getCatalog = async (req, res, next) => {
  ...
  const volumes = [
    ...new Set(
      products
        .map((product) => product.price.map((price) => price.volume))
        .flat()
        .filter((el) => el)
    ),
  ];
  const categories = [
    ...new Set(products.map((product) => product.category).filter((el) => el)),
  ];

  const maxPrice = Math.max(
    ...products.map((el) => el.price.map((item) => item.value)).flat()
  );
