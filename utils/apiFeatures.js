class APIFeatures {
  ...
  filterVolume() {
    if (this.queryString.volume) {
      const volume = this.queryString.volume.split(',').map((v) => v.trim());
  ...
  filterPrice() {
    if (this.queryString.price) {
      const price = this.queryString.price.split(',').map((p) => p.trim());
  ...
  filterCategory() {
    if (this.queryString.category) {
      const category = this.queryString.category.split(',').map((c) => c);
